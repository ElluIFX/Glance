#include "pch.h"
#include "component_localization.h"
...
namespace
{
    ...
    std::wstring resolve_language(const wchar_t* language_tag)
    {
        ...
        if (ResolveLocaleName(language_tag, resolved, std::size(resolved)) == 0)
        {
            return L"en-US";
        }
        return resolved;
...
{
    ComponentResourceStore::~ComponentResourceStore()
    {
        shutdown();
    }

    bool ComponentResourceStore::initialize() noexcept
    {
        try
        {
            ...
            if (FAILED(MrmCreateResourceManager(path.c_str(), &manager)) ||
                FAILED(MrmCreateResourceContext(manager, &context)) ||
                FAILED(MrmGetChildResourceMap(
                    manager,
                    nullptr,
                    L"Resources",
                    &resources)))
    ...
    void ComponentResourceStore::shutdown() noexcept
    {
        std::scoped_lock lock(mutex_);
        resources_ = nullptr;
        if (context_ != nullptr)
    ...
    bool ComponentResourceStore::copy(
