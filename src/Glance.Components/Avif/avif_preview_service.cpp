#include "pch.h"
#include "avif_preview_service.h"
...
namespace
{
    ...
    constexpr std::wstring_view extensions[]{ L".avif" };
    ...
    void cleanup_legacy_directories() noexcept
    {
        try
        {
            ...
            const auto lease_root =
                std::filesystem::temp_directory_path() / L"Glance" / L"AvifPreview";
...
namespace glance::components::avif
{
        ...
        try
        {
            ...
            try
            {
                auto root = std::filesystem::temp_directory_path() /
                    L"Glance" / L"AvifPreview" / std::to_wstring(GetCurrentProcessId());
            ...
            const auto host = component_directory_value / L"Glance.AvifHost.exe";
