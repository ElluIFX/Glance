...
namespace
{
    ...
    constexpr std::array avif_extensions{ L".avif" };
        ...
        ComponentRegistration* registration) noexcept
    {
        ...
        wcscpy_s(result.component_id, L"avif");
    ...
    {
        return path != nullptr &&
            glance::components::avif::can_preview(path);
    }

    PrepareStatus copy_preview_result(
        const glance::components::avif::PreviewResult& result,
        const wchar_t* language_tag,
        PreparedPreview* preview) noexcept
    {
        ...
        {
            glance::components::avif::release_preview(result.lease_token);
    ...
    {
        glance::components::avif::shutdown();
