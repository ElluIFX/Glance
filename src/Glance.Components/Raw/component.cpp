...
namespace
{
    ...
    constexpr std::array raw_extensions{
        ...
        ComponentRegistration* registration) noexcept
    {
        ...
        glance::components::raw::initialize();
        for (const auto* extension : raw_extensions)
        {
        ...
        wcscpy_s(result.component_id, L"raw");
    ...
    {
        return path != nullptr &&
            glance::components::raw::can_preview(path);
    }

    PrepareStatus copy_preview_result(
        const glance::components::raw::PreviewResult& result,
        PreparedPreview* preview) noexcept
    {
        ...
        {
            glance::components::raw::release_preview(result.lease_token);
        ...
        PreparedPreview* preview) noexcept
    {
        ...
        return copy_preview_result(
            glance::components::raw::prepare_preview(path, preview_dimension),
            preview);
    }

    void WINAPI release_preview(std::uint64_t lease_token) noexcept
    {
        glance::components::raw::release_preview(lease_token);
    }
        ...
        const wchar_t* path, const PreviewPreparationOptions* options, PreparedPreview* preview) noexcept
    {
        ...
        return copy_preview_result(
            glance::components::raw::prepare_preview(path, dimension), preview);
    }

    BOOL WINAPI can_refine(std::uint64_t token) noexcept
    {
        return !glance::components::raw::refinement_source(token).empty();
    }
        ...
        std::uint64_t token, const PreviewPreparationOptions*, PreparedPreview* preview) noexcept
    {
        if (preview == nullptr || preview->size < sizeof(PreparedPreview))
            return PrepareStatus::failed;
        const auto source = glance::components::raw::refinement_source(token);
        if (source.empty()) return PrepareStatus::unavailable;
        return copy_preview_result(
            glance::components::raw::prepare_preview(source, preview_dimension), preview);
    }
    ...
    GalleryMediaKind WINAPI classify_gallery_extension(const wchar_t* extension) noexcept
    {
        ...
        {
            for (const auto candidate : raw_extensions)
            {
                if (_wcsicmp(extension, candidate) == 0)
                {
    ...
    {
        return glance::components::raw::query_metadata(lease_token, sink);
    }
    ...
    {
        glance::components::raw::shutdown();
    }
}
