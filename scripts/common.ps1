...
function Stop-GlanceProcesses {
    $processes = Get-Process Glance, Glance.Core, Glance.DialogBroker32, Glance.OfficeHost, Glance.PdfHost, Glance.AdobeHost, Glance.ArchiveHost, Glance.HeicHost, Glance.AvifHost, Glance.RawHost, Glance.ExecutableHost, Glance.FontHost -ErrorAction SilentlyContinue
    if (-not $processes) {
        return
    }
